...
return_type aes_set_key_( const unsigned char key[], length_type keylen, aes_context ctx[1] )
{
	uint_8t cc, rc, hi;
...
return_type aes_encrypt_( const unsigned char in[N_BLOCK], unsigned char  out[N_BLOCK], const aes_context ctx[1] )
{
	if( ctx->rnd )
	{
...
return_type aes_cbc_encrypt_( const unsigned char *in, unsigned char *out,
						 int n_block, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{
	...
	{
		xor_block(iv, in);
		if(aes_encrypt_(iv, iv, ctx) != EXIT_SUCCESS)
			return EXIT_FAILURE;
...
return_type aes_decrypt_( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK], const aes_context ctx[1] )
{
	if( ctx->rnd )
	{
		uint_8t s1[N_BLOCK], r;
...
return_type aes_cbc_decrypt_( const unsigned char *in, unsigned char *out,
						 int n_block, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{
	while(n_block--)
	{   uint_8t tmp[N_BLOCK];
		...
		block_copy(tmp, in);
		if(aes_decrypt_(in, out, ctx) != EXIT_SUCCESS)
			return EXIT_FAILURE;
		xor_block(out, iv);
